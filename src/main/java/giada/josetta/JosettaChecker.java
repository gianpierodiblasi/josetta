...
//    if (classOrInterface.isInnerClass() && !classOrInterface.getNameAsString().matches("^\\$.*\\$$")) {
...
//    Map<String, Long> map = classOrInterface.getMethods().stream().filter(method -> !method.getNameAsString().matches("^\\$.*\\$$")).collect(Collectors.groupingBy(MethodDeclaration::getNameAsString, Collectors.counting()));
